...
- `src/muselens/index.py`：统一索引接口、默认 mmap 低内存精确检索，以及 NumPy / FAISS 对照后端。
- `src/muselens/tags.py`：受控中英文标签词表，以及复用 SigLIP2 向量的零样本标签器。
...
- 默认把连续 float32 向量写入可重建的 mmap 缓存，分块执行精确余弦检索；SQLite 是数据源，
  缓存损坏或丢失时在启动阶段流式恢复。NumPy / FAISS 保留为对照后端，数据扩大到至少
  50 万张后再判断是否需要 ANN 或独立向量服务。
- 图片导入到专用目录，不直接操作原始图片。
...
- `demo` 模式从固定种子恢复示例图库；持久化写接口返回 403。访客上传使用独立临时 API，
  每个会话拥有自己的文件目录、SQLite 元数据和最多 30 张图片的临时内存索引，不会污染示例图库。
...
- 编码器使用互斥推理和 100 项文本向量 LRU 缓存；临时图库批量编码图片，避免重复加载、
  并发推理竞争和逐张调用模型。
- 自动标签与语义检索共享同一个图片向量和模型；`image_tags` 表保存标签、显示名称、分数与
  词表模型版本。多选标签在同一筛选维度内采用 OR，和格式、方向、时间等维度共同采用 AND。
