...
- 免费公开演示、本地持久化图库和访客临时图库默认不设绝对门槛，始终保留最佳匹配，再用
  相对分差截断弱结果；前端明确将分数解释为查询内相对相关性。启用 Qwen3-VL 精排时才使用
  独立校准的拒答阈值，避免把重叠的 SigLIP2 原始分数伪装成可靠置信度。
- 编码器使用互斥推理和 100 项文本向量 LRU 缓存；临时图库批量编码图片，避免重复加载、
  并发推理竞争和逐张调用模型。
- 自动标签与语义检索共享同一个图片向量和模型；`image_tags` 表保存标签、显示名称、分数、
  词表模型版本和 `auto/manual` 来源。人工结果只在本地模式写入，批量重建与模型迁移会跳过；
  恢复自动则复用持久化向量。多选标签在同一筛选维度内采用 OR，和其他维度共同采用 AND。
- 智能相册由前端根据当前完整图库的持久化标签动态生成；空相册不展示，封面来自真实匹配图片，
  点击后复用后端组合筛选接口，因此不会复制照片或维护容易漂移的第二份相册状态。
- 自定义相册使用 `albums` 与 `album_images` 关系表，只保存图片 ID 引用。同一图片可进入多个相册；
  删除相册只级联删除关系，删除图片则自动清理失效关系。全部写接口沿用本地模式权限边界。
