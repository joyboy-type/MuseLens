...
- [x] 84 条中英文正例 + 10 条图库外查询的公开演示检索契约
...
查询，并要求 Hit@5 不低于 90%；完整合同运行全部 84 条正例，独立评测另含 10 条图库外查询：
...
公开演示检索合同现包含 84 条中英文图库内查询和 10 条图库外查询。2026-07-22 对线上轻量
SigLIP2 实测 Top-1 为 72.62%、Top-5 为 95.24%（英文 97.62%、中文 92.86%），图库外拒绝率
仍为 0%；加入 Qwen3-VL 精排后，旧版 44 条合同的无阈值 Top-1
